...
distribution as its stationary distribution. The chain is sampled by
generating proposals and accepting or rejecting them using a
Metropolis-Hasting acceptance rule. During an initial warm-up stage, the
parameters of the proposal distribution can be adapted, with adapters
available to both: tune the scale of the proposals by coercing the
average acceptance rate to a target value; tune the shape of the
...
(Gaussian) random walk and Langevin proposals. Optionally, if
[BridgeStan’s R
...
the `proposal` and `adapters` arguments to
[`sample_chain()`](http://github-pages.ucl.ac.uk/rmcmc/reference/sample_chain.md),
...
`target_distribution` argument to
[`sample_chain()`](http://github-pages.ucl.ac.uk/rmcmc/reference/sample_chain.md)
is passed a formula specifying the log density of the target
distribution, which is passed to
...
to construct necessary functions, using
[`stats::deriv()`](https://rdrr.io/r/stats/deriv.html) to symbolically
compute derivatives.
...
![](reference/figures/README-banana-samples-1.png)
